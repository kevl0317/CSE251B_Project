"""
GPT model with Muon optimizer (momentum + Newton-Schulz orthogonalization).
The model architecture is identical to the baseline; only the optimizer changes.

Muon uses momentum updates with Newton-Schulz iteration to orthogonalize
gradients for 2D weight matrices, which improves training dynamics.
1D parameters (biases, norms) use standard momentum SGD.

Reference: Keller Jordan et al. "Muon: An optimizer for matrix-like parameters"
"""
...
class Muon(torch.optim.Optimizer):
    ...
    standard momentum SGD for 1D params. Includes decoupled weight decay.
    """
...
class GPT(nn.Module):
    ...
    def configure_optimizers(self, weight_decay, learning_rate, betas, device_type):
        """
        Create a Muon optimizer. Muon uses momentum (from betas[0]) and
        applies Newton-Schulz orthogonalization to 2D weight matrices.
        A small amount of weight decay is applied via decoupled weight decay.
        """
        param_dict = {pn: p for pn, p in self.named_parameters()}
        param_dict = {pn: p for pn, p in param_dict.items() if p.requires_grad}

        # Separate 2D params (weights) from 1D params (biases, norms)
