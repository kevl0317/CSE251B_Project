"""
GPT model with Rotary Position Embeddings (RoPE) replacing absolute position embeddings.
RoPE encodes position information by rotating query and key vectors, providing
better length generalization and relative position awareness.
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Precompute RoPE frequency embeddings
        theta = 10000.0
...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type, override_args=None):
        ...
        # Skip wpe since RoPE has no position embeddings
        sd_keys_hf = [k for k in sd_keys_hf if not k.endswith('.wpe.weight')]
        transposed = ['attn.c_attn.weight', 'attn.c_proj.weight', 'mlp.c_fc.weight', 'mlp.c_proj.weight']
        # Filter out cos/sin buffers from our model's keys since HF doesn't have them
