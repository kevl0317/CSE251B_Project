...
class SwiGLU(nn.Module):
    ...
    # Hidden dim defaults to ~(8/3)*n_embd to match the vanilla MLP's 8*d^2 param budget;
    # set config.mlp_hidden_dim explicitly to control it. Down projection is named c_proj
    # so the residual scaled-init in GPT.__init__ (matches "c_proj.weight") still applies.
    def __init__(self, config):
        super().__init__()
...
class GPTConfig:
    ...
    use_swiglu: bool = False # if True, replace GELU MLP with LLaMA-style SwiGLU
    mlp_hidden_dim: int = 0  # SwiGLU hidden dim; 0 = auto (~8/3 * n_embd) for matched param count
