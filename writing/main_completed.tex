...
\\
% {\small $<$Your-accessible-and-up-to-date-GitHub-URL$>$} % Note: You can put it in your first page, if you prefer. Note 2: Use \href or \url to link your URL.
}
...
%
In this section, we clearly describe our empirical exploration of the language modeling problem on the FineWeb dataset and associated results. We provide evidence for our claims through systematic experimentation with different architectural modifications and regularization techniques.

\subsection{Baselines}
\label{sec:baselines}
We compare several baseline models. This comparison helps us understand the impact of different architectural choices and regularization techniques.

We compare with the following baselines:
\begin{itemize}
    \item \textbf{GPT-Base}: A standard 8-layer GPT architecture with 8 attention heads and 512-dimensional embeddings, using standard ReLU activations. This model serves as our reference point with 50.93M parameters.
    \item \textbf{GPT-Dropout}: The GPT architecture with dropout regularization (p=0.1). Dropout prevents overfitting during training.
    \item \textbf{GPT-WeightDecay}: The GPT architecture with L2 regularization (weight decay=0.1). Weight decay penalizes large weights.
    \item \textbf{GPT-SwiGLU}: The GPT architecture with SwiGLU activation function replacing ReLU. SwiGLU may improve gradient flow.
\end{itemize}

\subsection{Evaluation}
\label{sec:evaluation}
We evaluate all models on the validation set. We use perplexity and average loss (nats) as our primary metrics. Lower perplexity indicates better performance. We evaluate on 5,169,152 tokens with a context window of 1024 tokens.

\subsection{Implementation details}
\label{sec:implementation_details}
\paragraph{Computational Platform.} We conducted all experiments on NVIDIA CUDA-enabled GPUs. The baseline model evaluation takes 27.4 seconds for the full validation set. Modified models range from 18.2 to 29.9 seconds depending on architectural complexity.

\paragraph{Hyperparameters and Optimization.} We trained all models with the following configuration:
\begin{itemize}
    \item \textbf{Optimizer}: AdamW with cosine learning rate decay
    \item \textbf{Learning rate}: $6 \times 10^{-4}$ with minimum learning rate of $6 \times 10^{-5}$
    \item \textbf{Batch size}: 12 with gradient accumulation of 8 steps (effective batch size: 96)
    \item \textbf{Training iterations}: 10,000 iterations with 200 iteration warmup
    \item \textbf{Context length}: 1024 tokens
    \item \textbf{Architecture}: 8 layers, 8 attention heads, 512 embedding dimensions
    \item \textbf{Precision}: bfloat16 with compilation enabled for faster training
    \item \textbf{Evaluation}: Every 500 iterations with 100 evaluation iterations
\end{itemize}

\paragraph{Hyperparameter Choices.} We chose the learning rate and warmup period based on standard practices for training transformer models. GPU memory limitations constrained the batch size. We used gradient accumulation to maintain effective batch size. The 10,000 iteration training schedule balances computational constraints with convergence requirements. This limited iteration count is intentional for the milestone submission. For context, 10,000 iterations covers less than one full epoch on the FineWeb dataset. We plan to train the final model for significantly more iterations to allow for better convergence.

\subsection{Results}
\label{sec:results}
\cref{tab:results} presents the comparison of all baseline models on the validation set. Each architectural modification shows distinct trade-offs in performance and efficiency.

\begin{table}[h]
\caption{Validation set performance on FineWeb. All models trained with identical configurations except noted modifications. Perplexity lower is better.}
\label{tab:results}
\centering
\begin{tabular}{lcccc}
\toprule
Model & Parameters & Perplexity & Avg Loss (nats) & Eval Time (s) \\
\midrule
GPT-Base & 50.93M & 41.72 & 3.731 & 27.4 \\
GPT-Dropout & 50.93M & 43.99 & 3.784 & 20.5 \\
GPT-WeightDecay & 50.93M & 41.50 & 3.726 & 18.2 \\
GPT-SwiGLU & 51.06M & 41.20 & 3.718 & 29.9 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Impact of Regularization Techniques.} GPT-WeightDecay provides a modest improvement over GPT-Base (41.50 vs 41.72 perplexity). GPT-Dropout degrades performance (43.99 perplexity). L2 regularization is more effective than dropout for this model size and dataset. GPT-Base may not overfit significantly. The dropout rate may need further tuning.

\paragraph{Activation Function Improvements.} GPT-SwiGLU achieves the best performance (41.20 perplexity, 3.718 average loss). This represents a 1.2\% improvement in perplexity over GPT-Base. GPT-SwiGLU adds only 131K parameters (0.26\% increase). SwiGLU provides better gradient propagation than ReLU.

\paragraph{Computational Efficiency.} GPT-WeightDecay shows the fastest evaluation time (18.2s). GPT-SwiGLU is slower (29.9s) due to increased computational complexity. GPT-Dropout shows intermediate computational cost (20.5s).

\paragraph{Result Analysis.} Architectural modifications provide meaningful improvements with minimal parameter increases. GPT-SwiGLU achieves the best perplexity with small increases in model size and computational cost. GPT-WeightDecay provides a simpler regularization alternative that outperforms GPT-Dropout.
